...
### 本番Supabaseに自分のLINE User IDでダミーデータを投入する（実機テスト用）

実際に LINE アプリから LIFF を開いてスタンプカードを確認したい場合、`scripts/seed-line-user.ts` で本番 Supabase に自分の LINE User ID のダミー顧客データを投入できます。

```bash
npm run seed:line-user -- --line-user-id=Uxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx --total=47
```

| オプション | 必須 | 説明 |
|-----------|:---:|------|
| `--line-user-id` | ✅ | 自分の LINE User ID（LINE Developers Console のプロフィールや、LIFFアプリのログから確認できます） |
| `--total` | ✅ | 累計パン個数 |
| `--milestone` | - | `last_milestone_reached`（省略時は total から自動計算） |
| `--shopify-customer-id` | - | Shopify顧客ID（デフォルト: `9999900001`。実顧客と衝突しないダミー帯） |

- 環境変数は `.env.production.local`（推奨）または `.env.local` に `SUPABASE_URL`（未設定時は `NEXT_PUBLIC_SUPABASE_URL` で代用）と `SUPABASE_SERVICE_ROLE_KEY` を設定してください。**本番のService Role Keyを扱うため、これらのファイルは絶対にコミットしないこと**（`.gitignore` で除外済み）。
- 指定した LINE User ID が既に `customers` に存在する場合は累計・マイルストーンを更新し、存在しない場合は新規作成します。
